...
const AccordionTrigger = React.forwardRef(({ className, children, ...props }, ref) => {
    ...
    return (_jsxs("button", { ref: ref, type: "button", className: cn(`flex flex-1 items-center justify-between py-4 text-[11px] sm:text-sm md:text-lg text-blue-500 font-bold transition-all hover:underline 
...
const AccordionContent = React.forwardRef(({ className, children, ...props }, ref) => {
    ...
    const { value: itemValue } = itemContext;
    const isOpen = values.includes(itemValue);
    return (_jsx("div", { ref: ref, className: cn("overflow-hidden text-white text-[10px] sm:text-sm md:text-lg transition-all", isOpen ? "animate-accordion-down" : "animate-accordion-up h-0", className), "data-state": isOpen ? "open" : "closed", ...props, children: _jsx("div", { className: cn("pb-4 pt-0"), children: children }) }));
});
